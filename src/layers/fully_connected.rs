  ...
  pub fn random_vector(
    ...
    for idx in 0..size {
      ...
      if !constants.contains_key(&idx) {
        info!("Random vector is too small: {:?}", size);
      }
...
impl<F: PrimeField> Layer<F> for FullyConnectedChip<F> {
  fn forward(
    ...
    // Compute r1 * result
    let mut r1_res = vec![];
    // info!("r1_ref: {:?}", r1_ref.len());
    // info!("r2_ref: {:?}", r2_ref.len());
    // info!("mm_result: {:?}", mm_result.shape());
    ...
    // info!("r1_res_r2: {:?}", r1_res_r2);

    // Compute r1 * input
    let mut r1_input = vec![];
    // info!("input: {:?}", input.shape());
    // info!("r1_ref: {:?}", r1_ref.len());
    ...
    // info!("r1_inp_weight_r2: {:?}", r1_inp_weight_r2);
